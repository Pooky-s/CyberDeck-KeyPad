...
/* Debug vars */
  ...
  void testing_things(uint8_t number){
    uint8_t digits[2];
    if (number == 0) {
      ...
      for(int i = 1; i >= 0; --i)
        {
          ...
          digits[i] = display.encodeDigit(digit);
          number /= 16;
        }
    }
    const uint8_t SEG_DONES[] = {
      0x3f,   // O
      0x76,   // x
      digits[0],
      digits[1]
...
byte colPins[COLS] = {10, 11, 12}; //connect to the column pinouts of the kpd
...
void setup() {
  ...
  delay(500);
  Serial.printf("Layout : %d %d %x\n",KEY_LAYOUT, LAYOUT, keymap[0]);
}
    ...
    for (int i=0; i<LIST_MAX; i++)   // Scan the whole key list.
    {
      if ( kpd.key[i].stateChanged )   // Only find keys that have changed state.
      {
        switch (kpd.key[i].kstate) {  // Report active key state : IDLE, PRESSED, HOLD, or RELEASED
          ...
          #ifdef DEBUG
            testing_things(getKeyMapped(kpd.key[i].kchar));
          #endif
          msg = " PRESSED.";
        break;
          case HOLD:
